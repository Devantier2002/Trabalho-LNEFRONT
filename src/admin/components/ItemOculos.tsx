...
export default function ItemOculos({ produto, produtos, setProdutos }: listaProdutoProps) {
  ...
  async function alterarDestaque() {
    ...
    if (response.status == 200) {
      const produtos2 = produtos.map(x => { if (x.id == produto.id) { return { ...x, destaque: !x.destaque } } return x })
      setProdutos(produtos2)
    }
  }

  return (
    <tr key={produto.id} className="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:gray-800 border-b dark:border-gray-700">
